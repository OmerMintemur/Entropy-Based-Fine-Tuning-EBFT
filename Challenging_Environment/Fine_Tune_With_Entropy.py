...
layers_to_unfreeze = ['features.4.squeeze', 'features.6.squeeze', 'features.6.expand1x1', 'features.6.expand3x3',
                      'features.7.squeeze', 'features.7.expand1x1', 'features.7.expand3x3', 'features.9.squeeze',
                      'features.9.expand1x1', 'features.9.expand3x3', 'features.10.squeeze', 'features.10.expand1x1',
                      'features.10.expand3x3', 'features.11.squeeze', 'features.11.expand1x1', 'features.11.expand3x3',
                      'features.12.squeeze', 'features.12.expand1x1', 'features.12.expand3x3']
time_log = {}
RUNS = 5
# For faster training
...
for BATCH_SIZE in [8, 16, 32]:
    for run in range(0, 1):
        ...
        # 2) Unfreeze the selected feature layers
        for name, param in model.named_parameters():
            # any param whose name starts with one of these prefixes will be trainable
            if any(name.startswith(layer_name) for layer_name in layers_to_unfreeze):
                param.requires_grad = True

        # 3) Unfreeze final classifier head (4-class conv + anything else in classifier)
        for name, param in model.named_parameters():
        ...
        time_end = time.time()
        print(
            f"BATCH SIZE is : {BATCH_SIZE} -- RUN {run} -- Time elapsed during training is {time_end - time_begin:.2f}")
        ...
        df.to_csv(f"Challenge_Environment_Fine_Tuning_With_Entropy_Results_{BATCH_SIZE}.csv")
        ...
        time_end = time.time()
        print(
            f"BATCH SIZE is : {BATCH_SIZE} -- RUN {run} -- Time elapsed during inference is {time_end - time_begin:.2f}")
        ...
        for phase in ['test']:
            for inputs, labels in dataloaders[phase]:
                ...
                idx += batch_size
        model_save_path = f'fine_tuning_with_entropy_batch_{BATCH_SIZE}.pth'
        ...
        with open(f'predictions_labels_fine_tuning_with_entropy_batch_{BATCH_SIZE}.pkl', 'wb') as f:
            pickle.dump({
