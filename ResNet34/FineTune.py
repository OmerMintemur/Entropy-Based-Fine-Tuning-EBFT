...
for BATCH_SIZE in [8, 16, 32]:
    for run in range(0, RUNS):
        ...
        df.to_csv(f"Resnet_34_Full_Fine_Tuning_Results_Briscs_Batch_{BATCH_SIZE}_run_{run}_.csv")
        ...
        for phase in ['test']:
            for inputs, labels in dataloaders[phase]:
                ...
                idx += batch_size
        model_save_path = f'Resnet34_full_fine_tuning_batch_{BATCH_SIZE}_run_{run}_Briscs_Model.pth'
        ...
        with open(f'Resnet34_predictions_labels_full_fine_tuning_batch_{BATCH_SIZE}_run_{run}_Briscs.pkl', 'wb') as f:
            pickle.dump({
