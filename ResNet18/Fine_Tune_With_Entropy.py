...
layers_to_unfreeze = ['layer1.0.conv1','layer3.0.conv1','layer3.1.conv1','layer3.1.conv2','layer4.0.conv1',
                      'layer4.0.conv2', 'layer4.0.downsample.0', 'layer4.0.downsample.1', 'layer4.1.conv1',
                      'layer4.1.conv2']
time_log = {}
RUNS = 5
for BATCH_SIZE in [8, 16, 32]:
    for run in range(0, RUNS):
        # Load the data
        data_dir = 'C:\\Users\\7950x\\Desktop\\Final_Codes\\Brain Tumor MRI Dataset'
        ...
        model = return_resnet18_modified(mappings)
        # Open only specific layers
        # 3️⃣ Unfreeze corresponding BatchNorm layers too
        # This includes batchnorms in same blocks (e.g. layer4.0.bn1, bn2, downsample.1, etc.)
        for name, param in model.named_parameters():
            if any(layer in name for layer in layers_to_unfreeze) or any(
                    name.startswith(layer.replace('conv', 'bn')) for layer in layers_to_unfreeze
            ):
                param.requires_grad = True

        # 4️⃣ Additionally, ensure BN in downsample is unfreezed (already covered by list, but just to be safe)
        for name, module in model.named_modules():
            if any(layer in name for layer in layers_to_unfreeze) and isinstance(module, torch.nn.BatchNorm2d):
        ...
        time_log [f'Resnet18_Batch_Size_{BATCH_SIZE}_Run_{run}_Training_Time'] = time_end - time_begin
        ...
        df.to_csv(f"Resnet_18_Fine_Tuning_With_Entropy_Results_Kaggle_Batch_{BATCH_SIZE}_run_{run}_.csv")
        ...
        time_log[f'Resnet18_Batch_Size_{BATCH_SIZE}_Run_{run}_Inference_Time'] = time_end - time_begin
        ...
        for phase in ['test']:
            for inputs, labels in dataloaders[phase]:
                ...
                idx += batch_size
        model_save_path = f'Resnet18_fine_tuning_with_entropy_batch_{BATCH_SIZE}_run_{run}_Kaggle_Model.pth'
        ...
        with open(f'Resnet18_predictions_labels_fine_tuning_with_entropy_batch_{BATCH_SIZE}_run_{run}_Kaggle.pkl', 'wb') as f:
            pickle.dump({
        ...
        training_inference_time.to_csv("Resnet_18_Time_Logs_Kaggle.csv", encoding='utf-8', index=False)
